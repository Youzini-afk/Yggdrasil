...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1 + Experience Beta 2 + Experience Beta 3 + Experience Beta 4 + Experience Beta 5 + Experience Beta 6 + Performance & Code Health Beta（已完成）+ External Project Operating Plane Alpha Phase E2（已完成；E3–E6 执行中）。
- **Conformance：** 260 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** External Project Operating Plane Alpha Phase E2 已完成。`official/workspace-lab` 提供 workspace action policy boundary（deny-by-default fake executor）——describe contract、draft creation、explain permissions、request actions（全部默认拒绝、不真实执行、approval_token 不生效）、summarize audit（redacted，不含 raw env/logs/commands/secrets）。下一步是 E3 Managed Workspace Deterministic Proof。临时计划见 `docs/roadmap/EXTERNAL_PROJECT_OPERATING_PLANE_ALPHA.md`；外部调研证据位于 `/tmp/opencode/ygg-external-project-plane-20260520/`。红线：external project 不是 package，managed workspace 不是 kernel object，adapter/wrapper 才是普通 package；危险动作必须 policy/proposal/audit gated。
