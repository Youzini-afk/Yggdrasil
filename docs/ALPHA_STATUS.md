...
- **Conformance：** 120 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Agent Infrastructure Alpha 与 Model Provider Integration Alpha 已完成并文档化；Live Model Calls Alpha L2 已实现（LiveHttpOutboundExecutor：reqwest + rustls、HTTPS-only、redirect none、默认关闭、shape-only 审计），仍需 L3+ 推进公开 outbound/secret boundary 和真实 provider call，且必须通过普通 provider package 路径，不能变成 kernel model/prompt 语义或中转站后台。
