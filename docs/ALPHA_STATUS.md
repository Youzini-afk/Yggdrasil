...
- **conformance：** 398 个具名 CLI 用例通过，外加 crate 与 service 单元测试；105 个 v1 schema（57 methods + 41 events + 7 top-level）验证通过。
...
- Contract V1 是公开平台规范：57 个协议方法、41 个事件类型、105 个 JSON Schema。`kernel.v1.cap.*`、`kernel.v1.audit.package`、能力句柄、bindings 注入、Path B、conformance kit 与 SDK 生成均为 implemented。
...
- **本地加密 secret store：** `secret_ref:store:NAME` 通过 `StoreSecretResolver` 从 `~/.yggdrasil/secrets.dat` 解析；store 使用 age(X25519) 加密，主密钥来自 OS keyring（延后启用）或 0600 本地 key 文件。
...
- `docs/spec/v1/schemas/` 是 SDK 和 conformance 的单一可信源：57 methods、41 events、7 top-level，共 105 个 schema。
...
`cargo run -p ygg-cli -- conformance` 跑 398 个具名 CLI 用例。支持：
