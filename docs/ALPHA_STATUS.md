...
- **当前主线：** Experience Beta 3 已完成；Yggdrasil 现在具备包拥有的体验可观测性实验室（`official/experience-observability-lab`）和 Forge 体验观测面板。后端提供 session health、package health、agent run health、proposal causal chain、failure breadcrumbs、cost/latency summary 和 guardrail/audit summary——全部从协议可见引用派生，不读取 SQLite 或 runtime internals。Web Forge 增加 Experience Health、Causal Chain、Failure Breadcrumbs、Cost/Latency、Asset Provenance、Guardrail/Audit Summary 等 public-protocol-only panels。playable-creation-board 新增 `summarize_experience_health` 并含可观测性交叉引用。10 个新 conformance 用例。未引入 `kernel.observability.*` 或 `kernel.experience.*`。下一阶段方向为 **Experience Beta 4**：记忆/知识包。
