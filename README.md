...
- 329 个具名 conformance 用例 + crate / service 单元测试，全部通过。
...
- `secret_ref` 引用、host 拥有的环境变量解析器与 allowlist。
- 网络权限声明，外发请求的审计与脱敏，公开的 `kernel.outbound.execute`。
- 真实 HTTPS 出站执行器（默认关闭，HTTPS-only，重定向 fail-closed）。
