...
| `kernel.v1.outbound.git_fetch` | implemented | 受 host policy 约束的公开 HTTPS git fetch。 |
...
- 方法 schema：`docs/spec/v1/schemas/methods/`（58）。
...
110 个 schema 必须通过 `cargo run -p ygg-cli --bin validate-schemas`。
