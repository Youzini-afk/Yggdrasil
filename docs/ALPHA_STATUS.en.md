...
- `official/model-provider-lab` — no-network request normalization across eight families (OpenAI, Anthropic, Gemini, OpenAI-compatible, OpenRouter, DeepSeek, xAI, Fireworks), profile validation rejecting raw secrets, fake/local invoke (all eight families: OpenAI chat/responses, Anthropic messages, Gemini generateContent, OpenAI-compatible chat, OpenRouter chat/responses, DeepSeek chat, xAI chat/responses, Fireworks chat/responses; auditable outbound_request_shape), error explanation, and echo. Invoke requires HTTPS base_url, rejects raw credential/header.
