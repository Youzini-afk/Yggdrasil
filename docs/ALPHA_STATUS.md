...
- **Conformance：** 138 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Live Model Calls Alpha L5 已实现（`kernel.outbound.execute` 支持通过公开 outbound 边界访问 OpenAI、Anthropic、Gemini 三种 live adapter shapes。OpenAI Chat Completions（`/v1/chat/completions`，Authorization bearer）和 Responses（`/v1/responses`，Authorization bearer，不同 body shape）。Anthropic Messages（`/v1/messages`，x-api-key secret header + anthropic-version 安全 static header）。Gemini generateContent（`/v1beta/models/{model}:generateContent`，x-goog-api-key secret header）。所有通过本地 loopback HTTP server 测试——不依赖公网。Missing secret fails closed（不发送请求）。`static_headers` 参数新增安全非 secret header 注入，严格 allowlist（仅 anthropic-version、content-type、accept 等少量 provider/version/format headers）并显式阻止 secret-bearing 名（Authorization、x-api-key、Cookie 等）——防止 `static_headers` 成为 secret bypass 或 host header override 路径。Provider `normalize_request` 对齐已验证所有三家 provider family。Response/audit/log 无 raw secret 泄漏。L4 功能（secret_headers injection、EnvSecretResolver、DeepSeek canary、SSE stream canary、opt-in live）继续完全支持。仍需 L6+ 推进真实 provider streaming through outbound boundary。）
