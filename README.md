...
- 347 个具名 conformance 用例 + crate / service 单元测试，全部通过。
...
- 网络权限声明，外发请求的审计与脱敏，公开协议方法 `kernel.outbound.execute` 与 `kernel.outbound.stream`。
- 真实 live HTTP 出站执行器（默认关闭；需 opt-in profile + provider env；HTTPS-only，重定向 fail-closed）。
- 子进程 TypeScript SDK 的 `kernelClient` 可从 subprocess 包发起受权限约束的 reverse kernel calls。
