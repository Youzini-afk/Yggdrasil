...
- Contract V1 是公开平台规范，80 methods + 59 events + 8 top-level = 147 个 schema 全部通过校验，450 conformance cases 通过。
- 分层迁移已完成前两步：v1 事实漂移修复，以及 Experimental Contract Registry / alias / 显式 profile-version 协商；后续按 [`CONTRACT_V2_MIGRATION.md`](CONTRACT_V2_MIGRATION.md) 继续推进 owner 双栈。
