...
- **Conformance：** 129 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Live Model Calls Alpha L4 已实现（`kernel.outbound.execute` 增加 `secret_headers` 参数支持 host-side secret header injection：params 可传 `{ "Authorization": {"secret_ref":"secret_ref:env:DEEPSEEK_API_KEY", "scheme":"bearer"}}` 格式，host 内部通过 `EnvSecretResolver` 解析并把 Authorization 注入 `LiveHttpOutboundExecutor` request headers，raw secret 不返回给 package/audit/error/response；`OutboundExecutorRequest` 新增 `secret_headers` 和 `resolved_secret_headers` 字段，`LiveHttpOutboundExecutor::build_headers` 注入 resolved secret headers；canary DeepSeek profile shape 通过 `model-provider-lab/normalize_request` 验证 endpoint/dialect/stream_family 正确；SSE stream canary 通过 `model-provider-lab/normalize_stream` 验证 DeepSeek delta_sse start→chunk→end lifecycle；local loopback HTTP server conformance 验证 Authorization header 真实到达 server 但 raw secret 不出现在 protocol response/audit/log；opt-in live conformance 仅在 `YGG_LIVE_MODEL_TESTS=1` 且 `DEEPSEEK_API_KEY` 存在时尝试真实调用，默认 conformance 不公网）。仍需 L5+ 推进真实 provider streaming through outbound boundary。
