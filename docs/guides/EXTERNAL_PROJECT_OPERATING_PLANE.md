...
development execute/recover 会在内存操作中携带认证身份，但绝不携带其凭据。它会在 Docker 与 managed-workspace 效应前刷新 Host access journal，并在阻塞验证结束后再次复核。撤销无法中断已经进入 executor 的单个效应，但会阻止所有后续用户效应，并把 durable recovery 状态留给显式获授权的操作者对账。
