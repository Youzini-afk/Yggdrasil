...
## Phase E3 — Managed Workspace Deterministic Proof — 已完成

目标：用 deterministic fixture 证明 workspace state/projection/log/entrypoint/patch flow，不做任意项目真实运行。

已交付：

- `workspace-lab` 增加 deterministic fixture workspace 能力：`create_fixture_workspace`、`inspect_workspace`、`read_workspace_metadata`、`plan_workspace_run`、`record_fixture_process_result`、`discover_workspace_entrypoints`、`draft_workspace_patch`。
...
- conformance：workspace projection、entrypoint discovery、bounded logs、patch proposal、no package registry pollution（7 个新增用例）。

验收：仍无 arbitrary shell；无真实 install/run；所有 write 仍 proposal-gated。

## Phase E4 — Web Project Aggregation UI — 已完成

目标：Home/Forge 能显示 External Projects / Managed Workspaces / risk / entrypoints / logs / adapter candidates，仍 public protocol-only。

已交付：

- `clients/web/src/projects/external-projects.ts` 增加 External Project aggregation model，调用 `project-intake-lab` 与 `workspace-lab` no-execution capabilities。
- Home/Play 增加 External Project Operating Plane rail，显示未适配 Git 项目与 fixture managed workspace 卡片。
- Forge 增加 External Projects / Managed Workspaces panel，展示 source intake、stack/lifecycle risk、workspace plan、fixture workspace、entrypoints、run plan、patch proposal、adapter candidate。
- Assistant drawer 增加 external project assistant lane，展示 inspect/draft patch/generate adapter plan 的 no-execution CTA。
- 未加载相关包时 graceful degrade；UI 不直接读 SQLite、runtime internals 或本地文件系统。

验收：Web TypeScript；workspace-lab conformance；doc links；diff check；UI public protocol-only。

## Phase E5 — Adapter / Wrapper Generation Proof — 已完成

目标：从 fixture workspace 生成可读、可检查、可替换的普通 adapter package 骨架，证明未适配外部项目可通过普通 package 路径包装成 Ygg package。

已交付：

- 扩展 `official/project-intake-lab` 新增 4 项 adapter/wrapper 生成预览能力（共 11 项）：
  - `generate_adapter_manifest_preview`：产出 Ygg package manifest 预览，不写文件。`adapter_package_id` 不可为 `official/`，不可含 path traversal 或 unsafe chars。Capability 必须属于 adapter namespace。权限默认最小（no network/filesystem/process）。
  - `generate_subprocess_wrapper_preview`：产出 TypeScript/Python subprocess wrapper 代码预览，含安全注释要求 future policy-gated executor / explicit approval。不写文件，不执行。
  - `generate_adapter_fixture_preview`：产出 package fixture input/output 样本，值已 redacted；无 raw secrets。
  - `check_adapter_readiness`：产出 readiness checklist（capability_namespace_ok、surface_coverage、permissions_minimal、fixture_present、no_raw_secrets、no_forbidden_namespace、needs_approval_for_execution）。
- 安全：raw secret 阻断；无文件系统写/读；无进程/网络；unsafe adapter id 被拒绝；official adapter id 被拒绝（防止伪装官方）；capability namespace mismatch 被拒绝；输出不含 forbidden kernel.project/workspace/git/npm/deploy/ide namespace。
- 示例 package：`examples/packages/external-project-adapter-preview/` 作为 preview fixture（thirdparty namespace，不执行）。
- Conformance：8 个新增用例（adapter manifest preview no write、rejects official adapter id、rejects path traversal adapter id、capability namespace mismatch rejected、wrapper preview no execution、fixture preview redacted、readiness checklist ok、e5 no forbidden namespace no raw secret）。project-intake-lab 共 16 个 conformance 用例。全仓共 275 个 conformance 用例。
- Web：`clients/web/src/projects/external-projects.ts` 更新以读取 adapter_manifest、adapter_wrapper、adapter_fixture、adapter_readiness 字段；E5 能力不可用时优雅降级。

验收：adapter 是 ordinary package；不自动 publish；不自动授予 network/secrets；adapter_package_id 不可为 official/。
