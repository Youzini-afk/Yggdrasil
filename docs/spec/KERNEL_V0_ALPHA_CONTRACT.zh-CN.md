...
| `kernel.capability.stream` | planned | 描述符标志存在；stream 执行不存在。 |
| `kernel.capability.cancel` | planned | 尚无进行中调用表。 |
...
| `kernel.permission.grant/revoke/list/audit` | partial | Host-dev 调用者可以向 human 或 assistant principal 授予/撤销有范围权限、列出授权、检查授予/撤销审计事件。持久授权 rehydration 和完整资源策略覆盖仍为下一步。 |
| `kernel.proposal.create/get/list/approve/reject/apply` | partial | 用于需要审批的游创变更的通用 proposal lifecycle。初始 apply 支持覆盖 `asset.put` 和 `projection.rebuild`；更广泛的事务和 revert/compensation 仍为下一步。 |
...
| `network.hosts` | planned | 当 subprocess/remote 执行存在时适用。 |
