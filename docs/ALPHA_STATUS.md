...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1 + Experience Beta 2 + Experience Beta 3 + Experience Beta 4 + Experience Beta 5 + Experience Beta 6 + Performance & Code Health Beta（已完成）+ External Project Operating Plane Alpha（已完成）+ Storage Backend Neutrality Alpha S2（已完成，S3 下一步）。
- **Conformance：** 291 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Storage Backend Neutrality Alpha S2 已完成（Package-Scoped Data Contract + `official/storage-lab` + 10 个 storage-lab conformance 用例）。S3 下一步：Blob / Asset Store Contract Proof。临时计划见 `docs/roadmap/STORAGE_BACKEND_NEUTRALITY_ALPHA.md`。
