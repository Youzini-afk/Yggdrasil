...
移动 PWA、Web/Desktop 与远程 CLI 已通过同一公开 Host API 使用 scoped device 身份和 project/target context，没有旁路写入接口。后续仍包括 artifact 细粒度权限/加密/保留/GC、长操作再授权、管理员批量撤销，以及更丰富但保持声明式的 verifier / sandbox。
