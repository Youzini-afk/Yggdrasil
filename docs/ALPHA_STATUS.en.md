...
- **Conformance:** 120 named CLI cases plus crate and service unit tests.
...
- **Current headline:** Agent Infrastructure Alpha and Model Provider Integration Alpha are complete and documented; Live Model Calls Alpha L2 is implemented (LiveHttpOutboundExecutor: reqwest + rustls, HTTPS-only, redirect none, disabled by default, shape-only audit), with L3+ advancing the public outbound/secret boundary and real provider calls, still requiring ordinary provider-package paths, not kernel model/prompt semantics or relay-admin backends.
