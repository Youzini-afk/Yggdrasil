...
- **Current headline:** Creative Inference Capability Alpha C2 delivered. C0 ADR and C1 transport-neutral inference capability contract (`sdk/typescript/inference-capability` + `docs/guides/INFERENCE_CAPABILITY_AUTHORING.md`) are complete; C2 non-HTTP fake local provider proof (`official/inference-local-lab` + 5 conformance cases) is complete; the next stage is C3 cloud adapter package reposition. Position remains: Yggdrasil ships API-first in the near term, but the architecture must not become API-shaped; cloud providers are ordinary adapter packages, not the platform model abstraction. Temporary plan: `docs/roadmap/CREATIVE_INFERENCE_CAPABILITY_ALPHA.md`.
