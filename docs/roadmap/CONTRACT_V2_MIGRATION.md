...
- [x] Phase 9：迁移客户端，并完成一个真实 Deprecated → Legacy Adapter 转换。
...
实施结果（2026-07-21）：仓库现已发布 [`PROTOCOL_COMMONS.md`](../spec/PROTOCOL_COMMONS.md) 与 `protocol-descriptor.schema.json`。`host.info` 只公开 Change、Shell Default 和 World Bundle Experimental 三个描述符；`ContractSelection.protocols[]` 会在 dispatch 前协商精确协议/Profile 版本，报告已声明的 Proposal adapter，并以结构化 `protocol_major_mismatch` 拒绝不支持的 major。`ygg conformance protocol` 分别输出 `ProtocolConformanceReport` 或 `ImplementationConformanceReport`，`ygg conformance package` 继续输出独立 Package 报告。Change 与 Shell 报告会执行协议拥有的向量；注册表校验要求官方和第三方声明使用同一组必需向量 ID。
...
实施结果（2026-07-21）：仓库现已发布 [`COMPONENT_IDENTITY.md`](../spec/COMPONENT_IDENTITY.md)，以及 component、package-envelope、composition-lock 三个 schema。显式 component ID 与 behavior digest 不再依赖 `PackageManifest.id`；package record、lifecycle event、capability discovery/result/receipt、`KernelEnv` 与 subprocess handshake 都携带 component evidence 和保守边界声明。安装计划与 lockfile 分别锁定 envelope digest、component/behavior digest、protocol profile 与 content root，lock 检查会独立报告各项漂移。旧 manifest 获得 package-scoped adapter identity。`contract:none` 始终属于 Foreign Capsule，不能声明 protocol conformance，在受支持 Path B entry 上仍可启动，并明确报告不保证 composability 与 portability。
...
Phase 9B 实施结果（2026-07-21）：Contract Registry `0.5.0` 将上述两条 legacy alias 从
Deprecated 转换为 Legacy Adapter，同时保留 `deprecated_in`、replacement 与 `support_until`
历史。Runtime 和 conformance 将它们冻结为共享 canonical handler/schema 的双向 identity
adapter，并用未知字段无损测试证明不再增加独立字段语义。HTTP RPC、host stdio 与 subprocess
reverse stdio 继续保持 result 等价，只在 envelope/header 附加
`ygg.contract.alias.legacy_adapter` 诊断。九个 Phase 至此全部完成，最后一项迁移完成定义已满足。
...
| `legacy` | alias 等价、无损转换、支持窗口与 Legacy Adapter lifecycle diagnostics |
...
Registry `0.5.0` 已通过 `kernel.v1.host.info` 与 `kernel.v1.target.list` 满足最后一项；本迁移计划完成。
