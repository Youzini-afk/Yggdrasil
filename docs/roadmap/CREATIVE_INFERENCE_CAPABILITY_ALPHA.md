...
## Phase C3 — Cloud adapter package reposition（已完成）
...
- conformance/status wording 从 “model provider abstraction” 改为 “cloud adapter coverage”。

## Phase C4 — Ygg-native inference proposal vertical slice（已完成）

目标：证明推理不是"prompt -> text response"，而是参与 Yggdrasil 创作运行时。

已交付：

- `packages/official/inference-playtest-lab`：Ygg-native inference proposal vertical slice 包。
- capabilities：`draft_proposal`、`inspect_proposal`、`branch_plan`、`explain_flow`。
- surfaces：`forge_panel` + `assistant_action`（需要 user_approval）。
- `crates/ygg-runtime/src/inproc/inference_playtest_lab.rs`：in-process handler 注册。
- conformance：5 个具名用例证明推理输出是 approval-gated proposal，不是 chat message。
  - `official.inference_playtest_lab_draft`：draft_proposal 产 proposal_draft，含 requires_user_approval=true、asset.put、source_inference provenance、无 raw secret、不是 chat message。
  - `official.inference_playtest_lab_inspect`：inspect_proposal 返回 risk/operations/permissions/provenance。
  - `official.inference_playtest_lab_reject_apply_denied`：被拒绝的 proposal 不能 apply。
  - `official.inference_playtest_lab_apply_and_branch`：approve/apply 成功，asset 被写入，branch_plan + fork 创建 branch，branch metadata 包含 proposal/source inference provenance。
  - `official.inference_playtest_lab_no_chat_kernel_terms`：输出不含 messages/prompt/chat/kernel.model 等术语。
- Forge profile autoload 加入该包。
- 流程闭环：session → inference-local-lab/invoke → inference-playtest-lab/draft_proposal → kernel.proposal.create → inspect_proposal → approve/reject → apply → branch_plan → kernel.session.fork。
- 不新增 kernel protocol methods，不新增 kernel.model/prompt/chat，proposal apply 继续使用现有 asset.put/projection.rebuild。
