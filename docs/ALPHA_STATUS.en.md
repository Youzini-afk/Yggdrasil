...
- **Conformance:** 138 named CLI cases plus crate and service unit tests.
...
- **Current headline:** Live Model Calls Alpha L5 is implemented (`kernel.outbound.execute` now supports OpenAI, Anthropic, and Gemini live adapter shapes through the public outbound boundary. OpenAI Chat Completions (`/v1/chat/completions`, Authorization bearer) and Responses (`/v1/responses`, Authorization bearer, different body shape). Anthropic Messages (`/v1/messages`, x-api-key secret header + anthropic-version safe static header). Gemini generateContent (`/v1beta/models/{model}:generateContent`, x-goog-api-key secret header). All tested through local loopback HTTP servers — no public internet dependency. Missing secrets fail closed (no request sent). `static_headers` param adds safe non-secret header injection with strict allowlist (only a tiny set such as anthropic-version, content-type, and accept for provider/version/format headers) and explicit blocking of secret-bearing names (Authorization, x-api-key, Cookie, etc.) — prevents `static_headers` from becoming a secret bypass or host header override path. Provider `normalize_request` alignment verified for all three provider families. No raw secret leaks in response/audit/log across all providers. L4 features (secret_headers injection, EnvSecretResolver, DeepSeek canary, SSE stream canary, opt-in live) remain fully supported. L6+ needed for real provider streaming through outbound boundary.)
