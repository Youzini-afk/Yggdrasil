...
| Phase 4 | In progress | Identity/observation, typed-worker, reachability driver, and typed deployment worker are implemented: durable enrollment/heartbeat/revoke, MAC-bound authority, dual ledgers, artifact/verifier, equivalent local/Agent deployment semantics, loopback actual-port receipts, and outcome-unknown recovery; Host actual-port leases and authenticated tunnel/private preview remain |
