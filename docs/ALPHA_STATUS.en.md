...
- The Host control plane remains separate from Contract V1: the root token is the root credential, while durable device grants attenuate authority through `observe`, `project_operate`, `deploy`, development, and `access_manage` scopes and can expire or be revoked. The mobile PWA exchanges a one-time HTTPS pairing token for a Secure/HttpOnly cookie; every mutation still crosses the same Host API / RPC. See [`architecture/HOST_REMOTE_ACCESS.md`](architecture/HOST_REMOTE_ACCESS.en.md).
...
- **Settings:** six panels, all wired to real data.
  ...
  - Host Access — current root/device identity, scopes, HTTPS pairing links, pending invitations, device expiry, and grant revoke; new invitations select only `observe` by default.
