...
- Web 项目控制台：显示 target / exec / port / proxy 诊断，以及 host-plane 的活动修订、恢复状态、修订历史和最近任务。若项目声明部署描述符，用户可显式选择 Host 认证或公开 route，再点击 Deploy / Stop、启动 Build & Deploy、恢复或回滚；默认保持 Host 认证。
...
- Remote Target Agent 已有 Candidate 基线：用户显式选择 target 后，类型化 Docker operation 在 Agent 本机只绑定 loopback，Host 通过带 target/route/lease/epoch 约束的认证 reverse tunnel 提供私有 preview，并可复用显式 public vhost。完整 Project Console/dev-to-deploy 接线与远端故障矩阵仍在 Phase 4/5；该能力不等于应用级身份系统或任意网络代理。
