...
The first Phase 4C slice now routes local versus agent drivers from `ExecutionTargetReachability`; no caller-provided network address can act as a driver fallback. `local` `health.probe` uses the same durable `Requested → Accepted → Running → terminal receipt` states and idempotent replay as a remote operation, while undeclared capabilities continue to fail closed. Deployment, actual port leases, authenticated tunnel/private preview remain later Phase 4C slices, and the Host loopback upstream boundary is unchanged.
