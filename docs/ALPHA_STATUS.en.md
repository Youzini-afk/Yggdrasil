...
- **Conformance:** 111 named CLI cases plus crate and service unit tests.
...
- `cargo run -p ygg-cli -- conformance` runs 111 named CLI cases covering: sessions, events, packages, capabilities, hooks, schemas, principals, permissions, subprocess execution, host transports, surfaces, proposals, official packages, composition-lab, asset-lab, projection-lab, persona-lab, knowledge-lab, context-lab, text-transform-lab, model-connector-lab, model-provider-lab, model-routing-lab, pi-agent-runtime-lab, capability-tool-bridge-lab, in-process package fallback hardening, playable-seed, blank play-creation loop, generated package authoring, composition descriptors, package check diagnostics, package reload smoke, third-party replacement proof, permission grant rehydrate, secret_ref validation, raw-secret blocking, official-package no-secret-bypass, network permission enforcement with audit, network policy checker pure function, **outbound executor boundary (denied request never reaches executor, policy/executor mismatch fails closed, allowlisted fake executor with network_performed:false, raw body not in audit, secret_refs as refs only, host mismatch redirect denied)**, streaming/cancellation lifecycle, generated template conformance, and no-network readiness proof.
