...
- `sdk/typescript/inference-capability`：transport-neutral envelope 与 stream/error/capability manifest helpers。
- `docs/guides/INFERENCE_CAPABILITY_AUTHORING.md`：包作者指南。
...
- resource hints；
- secret refs；
- transport kind hint（`http`、`local_process`、`in_memory`、`ipc`、`websocket`、`remote` 等）；
...
- conformance：无 URL、无 Authorization、无 HTTP status、无 provider schema 也能产生 deterministic stream frames。
