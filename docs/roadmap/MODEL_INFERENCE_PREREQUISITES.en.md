...
Model Connectivity Kit deliberately stopped before real model execution. Follow-up work turns these prerequisites into ordinary capability packages, SDKs, and a host outbound boundary. It does not become a relay gateway, billing system, or kernel model ontology.
...
1. **Secret resolution**
    ...
    - `SecretRef` supports `secret_ref:`, `secretRef:`, `secret-ref:`, and `host:` patterns. `HostSecretResolver` trait and `DenyAllSecretResolver` placeholder exist.
    - Raw-secret blocking is implemented in proposals and asset metadata. Official packages have no bypass. Permission grants survive rehydrate. Production vault integration remains host-level.
    - TypeScript `secretRef()`, `isValidSecretRef()`, `looksLikeRawSecret()`, and `isSecretFieldName()` helpers live in `sdk/typescript/secure-execution`. `--template networked` demonstrates `secretRef` usage. `examples/packages/faux-model-readiness/` uses `secret_ref` for credentials.
...
7. **Data redaction and approval**
   ...
   - A conservative redaction scanner covers known secret field names and value patterns in trusted paths such as proposal payloads and asset metadata. Outbound audit records use `RedactionState` with `not_captured`/`redacted`/`policy_ref`/`unsafe_blocked`/`explicitly_approved`. Content/description/title/reason fields are excluded from value-pattern scanning to avoid false positives.
...
None of these are part of the current prerequisites.
