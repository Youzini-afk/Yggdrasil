...
- **conformance：** 387 个具名 CLI 用例通过，外加 crate 与 service 单元测试；105 个 v1 schema（57 methods + 41 events + 7 top-level）验证通过。
...
`cargo run -p ygg-cli -- conformance` 跑 387 个具名 CLI 用例。支持：
