...
这些路由位于 Host 认证 middleware 内。root token 仍是完整 Host 门禁；paired device 通过 `develop_propose`、`develop_approve`、`develop_execute` 三个独立 scope 衰减权限，未知写操作 fail closed。`host serve` 绑定非 loopback 地址时仍必须提供非空 root token。设备 pairing、撤销、HTTPS 与 Cookie 边界见 [`HOST_REMOTE_ACCESS.md`](HOST_REMOTE_ACCESS.md)；项目级多租户 scope 仍是后续工作，不能通过给官方包加特权来替代。
...
移动 PWA 已通过同一 Host API 使用 scoped device 身份；它没有旁路写入接口。远程 CLI 专用 UX、项目级 scope、artifact 权限/GC，以及更丰富的验证器会在相同边界上继续演化。
