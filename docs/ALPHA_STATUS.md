...
- **conformance：** 418 个具名 CLI 用例通过，外加 crate 与 service 单元测试；114 个 v1 schema（62 methods + 45 events + 7 top-level）验证通过。
...
- Contract V1 是公开平台规范：62 个协议方法、45 个事件类型、114 个 JSON Schema。`kernel.v1.cap.*`、`kernel.v1.audit.package`、能力句柄、bindings 注入、Path B、conformance kit 与 SDK 生成均为 implemented。
...
- `docs/spec/v1/schemas/` 是 SDK 和 conformance 的单一可信源：62 methods、45 events、7 top-level，共 114 个 schema。
...
`cargo run -p ygg-cli -- conformance` 跑 418 个具名 CLI 用例。支持：
