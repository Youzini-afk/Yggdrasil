...
- **Conformance：** 160 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **Agentic Forge TypeScript SDK**（`sdk/typescript/agentic-forge/index.ts`）：Package-owned agent run lifecycle、plan graph、working state 和 candidate shape helper。`AgentRunLifecycleState` 枚举（9 个状态）。`PlanNode`/`PlanEdge`/`PlanGraph`/`WorkingState`/`RunEvent`/`CandidateShell`/`ObservabilitySummary` 类型。`createRunEvent`/`validatePlanGraph`/`createPlanGraph`/`createWorkingState`/`createCandidateShell` 构造器。`blockRawSecrets`/`looksLikeRawSecret`/`isSecretFieldName`/`hasKernelAgentNamespace` secret 安全与命名空间安全 helper。纯 TS 自测。输出不含 `kernel.agent.*`/`kernel.model.*`/`kernel.prompt.*`/`kernel.memory.*`/`kernel.turn.*`。
