...
- **Conformance:** 124 named CLI cases plus crate and service unit tests.
...
- **Current headline:** Agent Infrastructure Alpha and Model Provider Integration Alpha are complete and documented; Live Model Calls Alpha L3 is implemented (`kernel.outbound.execute` public protocol method: package principal determined from context (no spoofing), FakeOutboundExecutor + allowed network declaration succeeds with executor_kind Fake / network_performed false / host audit produced; spoofed package_id rejected; no network permission denied with executor not called; secret_refs as references only with no raw secret in response), with L4+ advancing real provider live calls, still requiring ordinary provider-package paths, not kernel model/prompt semantics or relay-admin backends.
