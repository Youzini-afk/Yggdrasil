...
- The platform UI has been rebuilt and release-closed: `clients/web` moved to React 19 + Tailwind v4 + Motion + Radix + Phosphor. Every screen (Home, the five Settings panels, the three-step Install flow, External wizard, Failure modal, Project frame, toast system) is implemented. Install uses ordinary `official/install-lab` capabilities for plan/execute, Failure Modal reads redacted package diagnostics, Disk Usage reads project `storage_summary`, and dark mode, responsive breakpoints, a11y, and lazy chunks have been tightened. Visual rules and the component map are in [`../design/PLATFORM_UI_DESIGN.md`](../design/PLATFORM_UI_DESIGN.en.md) and [`../../clients/web/README.md`](../../clients/web/README.md).
- Code organization closure is complete: runtime public-protocol handlers, `official/install-lab`, the Web Install/Home flows, the conformance registry, and the schema exporter are split into domain modules. This was behavior-preserving maintainability work and does not change the public protocol.
...
## Distribution polish (next release closure batch)
