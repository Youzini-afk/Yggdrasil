...
执行顺序见 [`MODEL_PROVIDER_INTEGRATION_ALPHA.md`](MODEL_PROVIDER_INTEGRATION_ALPHA.md)：provider API 调研 ledger（✅ M0）、`sdk/typescript/model-provider-adapter`（✅ M1）、`official/model-provider-lab` no-network normalization（✅ M2）、host outbound executor boundary（✅ M3）、OpenAI/Anthropic/Gemini invoke adapters、OpenAI-compatible/OpenRouter/DeepSeek/xAI/Fireworks presets、streaming normalization、examples/conformance/durable docs。
