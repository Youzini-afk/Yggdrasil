...
- Contract V1 is the public platform spec; all 154 schemas (80 methods + 59 events + 15 top-level) validate, and 465 conformance cases pass.
- The first six layering-migration steps are complete: v1 factual-drift repair, the Experimental Contract Registry with explicit negotiation, 36 owner-namespace canonical/legacy dual stacks, the SHA-256 object/artifact foundation, EffectReceipt / Change primitives, and the Protocol Commons descriptor/negotiation/conformance scaffold. The next step in [`CONTRACT_V2_MIGRATION.md`](CONTRACT_V2_MIGRATION.en.md) separates the package envelope from component identity.
