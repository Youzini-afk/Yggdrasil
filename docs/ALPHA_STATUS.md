...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1 + Experience Beta 2 + Experience Beta 3 + Experience Beta 4 + Experience Beta 5 + Experience Beta 6 + Performance & Code Health Beta（已完成）+ External Project Operating Plane Alpha Phase E4（已完成；E5–E6 执行中）。
...
- **当前主线：** External Project Operating Plane Alpha Phase E4 已完成。Web shell 新增 `clients/web/src/projects/external-projects.ts`，通过 public protocol/capability invoke 聚合 `official/project-intake-lab` 与 `official/workspace-lab` 的 no-execution 输出；Home/Play 显示 External Project Operating Plane rail，Forge 显示 External Projects / Managed Workspaces panel，Assistant drawer 显示 inspect/draft patch/generate adapter plan 的轻量入口。下一步是 E5 Adapter / Wrapper Generation proof。临时计划见 `docs/roadmap/EXTERNAL_PROJECT_OPERATING_PLANE_ALPHA.md`；外部调研证据位于 `/tmp/opencode/ygg-external-project-plane-20260520/`。红线：external project 不是 package，managed workspace 不是 kernel object，adapter/wrapper 才是普通 package；危险动作必须 policy/proposal/audit gated。
