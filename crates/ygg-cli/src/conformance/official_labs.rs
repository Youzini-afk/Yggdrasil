...
pub(crate) async fn package_installer_lab() -> anyhow::Result<()> {
    ...
    runtime
        .load_package(
            manifest::read_manifest(PathBuf::from(
                "packages/official/package-installer-lab/manifest.yaml",
            ))
            .await?,
        )
        .await?;
    let contract = runtime
        .invoke_capability(CapabilityInvocationRequest {
            handle: None,
            capability_id: Some(
                "official/package-installer-lab/describe_install_contract".to_string(),
            ),
            caller_package_id: None,
    ...
    anyhow::ensure!(
        contract
            .output
            .to_string()
            .contains("kernel.v1.outbound.git_fetch"),
    ...
    anyhow::ensure!(
        apply.output["lockfile_entry"]["commit_sha"]
