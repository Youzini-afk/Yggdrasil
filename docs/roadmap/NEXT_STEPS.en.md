...
- Contract V1 is the public platform spec; all 157 schemas (80 methods + 59 events + 18 top-level) validate, and 468 conformance cases pass.
- The first seven layering-migration steps are complete: v1 factual-drift repair, the Experimental Contract Registry with explicit negotiation, 36 owner-namespace canonical/legacy dual stacks, the SHA-256 object/artifact foundation, EffectReceipt / Change primitives, the Protocol Commons scaffold, and independent package-envelope/component identity with trust and composition pins. The next step in [`CONTRACT_V2_MIGRATION.md`](CONTRACT_V2_MIGRATION.en.md) proves the boundary with a real World Bundle.
