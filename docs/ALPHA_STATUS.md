...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1 + Experience Beta 2 + Experience Beta 3 + Experience Beta 4 + Experience Beta 5 + Experience Beta 6 + Performance & Code Health Beta（已完成）+ External Project Operating Plane Alpha Phase E3（已完成；E4–E6 执行中）。
- **Conformance：** 267 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** External Project Operating Plane Alpha Phase E3 已完成。`official/workspace-lab` 新增 deterministic fixture managed workspace 能力——create_fixture_workspace（managed_workspace_kind="fixture"、execution_performed=false、workspace_created_in_host=false、真实创建需要 approval/policy/executor）、inspect_workspace（不读磁盘）、read_workspace_metadata（不读磁盘）、plan_workspace_run（所有 run/install/test 步骤 requires_approval、executor_invoked=false）、record_fixture_process_result（只记录 fixture 结果 shape、real_process_spawned=false、raw 字段清除）、discover_workspace_entrypoints（根据 stack_hint/metadata/scripts 生成 entrypoint candidates、不扫描文件系统）、draft_workspace_patch（只生成 proposal shape、file_write_performed=false、unsafe local path 阻断、raw secret 阻断）。下一步是 E4 Web Project Aggregation UI。临时计划见 `docs/roadmap/EXTERNAL_PROJECT_OPERATING_PLANE_ALPHA.md`；外部调研证据位于 `/tmp/opencode/ygg-external-project-plane-20260520/`。红线：external project 不是 package，managed workspace 不是 kernel object，adapter/wrapper 才是普通 package；危险动作必须 policy/proposal/audit gated。
