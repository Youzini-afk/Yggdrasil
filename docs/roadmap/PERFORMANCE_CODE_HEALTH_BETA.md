...
已交付：
- `clients/web/src/main.ts` 新增 16ms render scheduler，SSE / action burst 不再直接连续触发整页 render。
- `clients/web/src/utils/html.ts` 新增 bounded JSON preview helper，限制 depth、array items、object keys 和 string length，避免大 payload 默认 stringify 全量展开。
- `clients/web/src/surfaces/forge.ts` 对 events、proposals、assets、projections、surfaces 做显示 cap，并把 event/proposal/surface/projection payload 改为 preview details。
- `clients/web/src/performance/render-diagnostics.ts` 新增 pure TS Forge render diagnostics helper，可用 50/500 mock events 记录 render HTML bytes 与 elapsed_ms。

验收：Web TypeScript 通过；public protocol-only；无 SQLite/runtime internals；500 events 的 Forge mock/render 指标记录到 baseline 文档。
