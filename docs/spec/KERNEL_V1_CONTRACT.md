...
- 顶层 schema：`docs/spec/v1/schemas/*.schema.json`（18），包含 additive Protocol Commons 与 component/package-envelope descriptor。
- 错误码：[`v1/ERROR_CODES.md`](v1/ERROR_CODES.md)。
- 事件 registry：[`v1/EVENT_KIND_REGISTRY.md`](v1/EVENT_KIND_REGISTRY.md)。

157 个 schema 必须通过 `cargo run -p ygg-cli --bin validate-schemas`。
