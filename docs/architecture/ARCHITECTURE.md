...
路径 A 包（`entry.contract: "v1"`）启动时收到 bindings。Subprocess 包在 `package.handshake` 中接收 bindings 字典；Rust in-process 包通过 `KernelEnv` 初始化；WASM 与 remote 将在 Round 10 通过 WIT resource imports 与 SPIFFE/Biscuit token 兑换补齐。Bindings 只包含该包被授予的最小权威。
