...
## External Project Operating Plane Alpha (E1 complete; E2–E6 in progress)
...
- **E1 — Project Intake Lab** (complete): `official/project-intake-lab` — 7 capabilities (describe_intake_contract / inspect_external_project_ref / detect_project_stack_from_metadata / draft_workspace_plan / draft_security_risk_summary / list_candidate_entrypoints / draft_adapter_plan), 3 surfaces (forge_panel / assistant_action / home_card), source classification (git/npm/local/archive/unknown), stack detection (node/rust/python/static/unknown), npm lifecycle risk detection (preinstall/install/postinstall/prepare/prepublish with executes_code/requires_approval), unsafe local path rejection (path traversal, home path, absolute sensitive paths), plan-only workspace plans and adapter plans, raw-secret blocking, no execution, no network, no filesystem, no kernel.project/workspace/git/npm/deploy/ide namespace. 8 conformance cases.
- **E2 — Workspace Action Policy Boundary**: `official/workspace-lab`, dangerous-action taxonomy, deny-by-default fake executor shape, and policy/audit/proposal gates.
- **E3 — Managed Workspace Deterministic Proof**: fixture workspace state/projection/log/entrypoint/patch flow without arbitrary real project execution.
- **E4 — Web Project Aggregation UI**: Home/Forge display external projects, workspaces, risk, entrypoints, logs, and adapter candidates, public-protocol-only.
- **E5 — Adapter / Wrapper Generation Proof**: `official/adapter-lab` generates ordinary subprocess adapter packages from fixture workspaces.
- **E6 — Durable cleanup**: delete the temporary plan and converge the external project operating-plane guide, ALPHA_STATUS, NEXT_STEPS, and CONFORMANCE_MATRIX.
