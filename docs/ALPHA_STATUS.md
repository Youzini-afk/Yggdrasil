...
- **conformance：** 371 个具名 CLI 用例通过，外加 crate 与 service 单元测试；110 个 v1 schema（58 methods + 45 events + 7 top-level）验证通过。
...
- Contract V1 是公开平台规范：58 个协议方法、45 个事件类型、110 个 JSON Schema。`kernel.v1.cap.*`、`kernel.v1.audit.package`、能力句柄、bindings 注入、Path B、conformance kit 与 SDK 生成均为 implemented。
...
- **协议方法：** `kernel.v1.outbound.audit` 列出某个能力包的出站审计事件；`kernel.v1.outbound.execute` 让普通能力包通过 host executor 发起一元出站请求；`kernel.v1.outbound.stream` 提供 SSE/NDJSON/raw 流式出站；`kernel.v1.outbound.websocket.open|send|close` 提供双向 WebSocket 出站；`kernel.v1.outbound.git_fetch` 让能力包在 host policy 下请求公开 HTTPS git fetch。
...
- `docs/spec/v1/schemas/` 是 SDK 和 conformance 的单一可信源：58 methods、45 events、7 top-level，共 110 个 schema。
...
`cargo run -p ygg-cli -- conformance` 跑 371 个具名 CLI 用例。支持：
