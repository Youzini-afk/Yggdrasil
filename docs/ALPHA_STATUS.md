...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1 + Experience Beta 2 + Experience Beta 3 + Experience Beta 4 + Experience Beta 5 + Experience Beta 6 + Performance & Code Health Beta（已完成）+ External Project Operating Plane Alpha Phase E1（已完成；E2–E6 执行中）。
- **Conformance：** 253 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** External Project Operating Plane Alpha Phase E1 已完成。`official/project-intake-lab` 提供静态 project intake：在不 clone/install/run/network/filesystem 的前提下 inspect external project refs、从 metadata detect stack、draft workspace plan、summarize security risk、list candidate entrypoints、draft adapter plan。下一步是 E2 Workspace Action Policy Boundary。临时计划见 `docs/roadmap/EXTERNAL_PROJECT_OPERATING_PLANE_ALPHA.md`；外部调研证据位于 `/tmp/opencode/ygg-external-project-plane-20260520/`。红线：external project 不是 package，managed workspace 不是 kernel object，adapter/wrapper 才是普通 package；危险动作必须 policy/proposal/audit gated。
