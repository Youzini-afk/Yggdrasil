...
| `kernel.v1.project.get` | implemented | HostAdmin/HostDev only；返回单个项目的完整 descriptor 与 registry 记录。 |
| `kernel.v1.project.start` | implemented | HostAdmin/HostDev only；把项目从 Installed/Stopped 切到 Running 并发出生命周期事件。 |
| `kernel.v1.project.stop` | implemented | HostAdmin/HostDev only；停止 Running 项目并发出生命周期事件。 |
| `kernel.v1.project.status` | implemented | HostAdmin/HostDev only；返回项目状态与最近错误。 |
...
- 方法 schema：`docs/spec/v1/schemas/methods/`（62）。
...
114 个 schema 必须通过 `cargo run -p ygg-cli --bin validate-schemas`。
