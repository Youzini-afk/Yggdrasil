...
- **Conformance:** 450 named CLI cases pass, plus crate and service unit tests; 147 v1 schemas validate (80 methods + 59 events + 8 top-level).
...
- The Experimental Contract Registry is implemented: `host.info` and `host.target.list` are the first canonical routes, legacy `kernel.v1.*` IDs resolve through centralized identity aliases to the same handlers, and RPC callers can require an exact profile/layer version. Unsatisfied selections return `unsupported_contract` without silent downgrade. Generated SDKs expose canonical clients and legacy wrappers.
...
`cargo run -p ygg-cli -- conformance` runs 450 named CLI cases. Flags:
