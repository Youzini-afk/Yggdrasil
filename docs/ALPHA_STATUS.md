...
- **conformance：** 362 个具名 CLI 用例通过，外加 crate 与 service 单元测试；105 个 v1 schema（57 methods + 41 events + 7 top-level）验证通过。
...
- 原先负责 git 安装实验的 installer lab 已移除；未来由 Round 10+ 的普通能力包 `official/git-tools-lab` 实现 git 安装流程。
...
`cargo run -p ygg-cli -- conformance` 跑 362 个具名 CLI 用例。支持：
