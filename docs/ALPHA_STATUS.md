...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0。
- **Conformance：** 187 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Agentic Forge Beta 与 Experience Beta 0 已完成；Yggdrasil 已具备 package-owned experience runtime contract——experience 描述符、state projection、checkpoint、recovery plan 与 Play/Forge/Assist surface 绑定——全部作为普通包 artifact，无内核体验语义。下一阶段方向为 **Experience Beta 1**：构建首个真实 playable vertical slice 以牵引后续底座建设。
