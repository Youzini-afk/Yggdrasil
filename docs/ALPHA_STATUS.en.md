...
- **Conformance:** 129 named CLI cases plus crate and service unit tests.
...
- **Current headline:** Live Model Calls Alpha L4 is implemented (`kernel.outbound.execute` now supports `secret_headers` param for host-side secret header injection: params accept `{ "Authorization": {"secret_ref":"secret_ref:env:DEEPSEEK_API_KEY", "scheme":"bearer"}}`, host resolves via `EnvSecretResolver` and injects Authorization into `LiveHttpOutboundExecutor` request headers, raw secrets never returned to package/audit/error/response; `OutboundExecutorRequest` extended with `secret_headers` and `resolved_secret_headers`; `LiveHttpOutboundExecutor::build_headers` injects resolved secret headers; canary DeepSeek profile shape validated through `normalize_request`; SSE stream canary through `normalize_stream` validates DeepSeek delta_sse start→chunk→end lifecycle; local loopback HTTP server conformance proves Authorization header arrives at server but raw secret stays out of protocol response/audit/log; opt-in live conformance only when `YGG_LIVE_MODEL_TESTS=1` and `DEEPSEEK_API_KEY` exist, default conformance skips). L5+ needed for real provider streaming through outbound boundary.
