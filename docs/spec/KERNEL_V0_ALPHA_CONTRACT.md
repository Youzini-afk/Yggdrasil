...
| `AssetRecord` | partial | 不透明 asset put/get/list 已存在，包含 id、origin package、mime、hash、size、metadata 和 `kernel/v1/asset.put` 审计事件。Asset 状态可以从持久事件日志 rehydrate；二进制/blob 存储和权限执行仍为下一步。 |

## 协议方法矩阵

| 方法 | 状态 | 备注 |
|---|---:|---|
| `kernel.v1.session.open` | implemented | 开启内容无关 session 并写入 `kernel/v1/session.opened`。 |
| `kernel.v1.session.close` | implemented | 关闭 session 并写入 `kernel/v1/session.closed`。 |
| `kernel.v1.session.fork` | partial | 从父序列创建子 session 并记录 branch 族系，不解释内容。 |
| `kernel.v1.session.branch.list` | partial | 列出与 session 相关的内存 branch 记录。 |
| `kernel.v1.session.get` | planned | 尚未在 service/CLI 中暴露。 |
| `kernel.v1.session.list` | planned | 尚未在 service/CLI 中暴露。 |
| `kernel.v1.event.append` | implemented | 对非内核 writer 强制执行 writer namespace 和 `events.append`。 |
| `kernel.v1.event.list` | implemented | 按 session 列出事件，支持 `after_sequence`、`limit`、`kind_prefix` 和 `writer_package_id`；运行时具备调用者感知的 `events.read` 门控，而 HTTP/CLI host 级别列表仍为 host-dev 本地管理。 |
| `kernel.v1.event.subscribe` | partial | HTTP SSE 端点从 `after_sequence` replay 并 tail 实时事件。协议方法分发和 package-principal subscribe 权限仍为下一步。 |
| `kernel.v1.package.load` | partial | 验证 manifest、host 策略，为 capability provider 解析 `rust_inproc` host entry，启动 subprocess JSON-RPC stdio entry，注册声明的 capability/hook，写入 lifecycle 事件。完整的过渡事件仍为 Platform Host Alpha 工作。 |
| `kernel.v1.package.unload` | partial | 在存在 subprocess 句柄时停止，移除注册记录和声明的 capability/hook，写入 lifecycle 事件。 |
| `kernel.v1.package.list` | implemented | 列出内存中的包记录。 |
| `kernel.v1.package.status` | implemented | 返回 package id 的注册记录。 |
| `kernel.v1.package.restart` | partial | 重启 subprocess entry 并发出 lifecycle 事件；其他 entry 形式被拒绝。 |
| `kernel.v1.package.logs` | partial | 排出捕获的 subprocess stderr 日志并发出 `kernel/v1/package.log` 事件；stdout 保留给 JSON-RPC 协议帧。 |
| `kernel.v1.package.describe` | planned | 可以从 status manifest 派生，但尚未作为方法暴露。 |
| `kernel.v1.capability.discover` | implemented | 列出已注册的描述符。 |
| `kernel.v1.capability.describe` | planned | 注册表可以检查描述符；协议方法尚未暴露。 |
| `kernel.v1.capability.invoke` | partial | 当提供调用者 package id 时强制执行调用者 capability 权限，除非提供 `provider_package_id` 否则检测模糊 provider，支持简单精确/主版本约束，根据支持的 schema 子集验证 capability input/output，通过 in-process package trait 执行 `rust_inproc` provider，并通过超时/降级处理执行 subprocess JSON-RPC stdio provider。 |
| `kernel.v1.capability.stream` | partial | 描述符标志存在；stream start/cancel 生命周期在内存注册表中可用并带有序列事件。真实网络 streaming 延后。 |
| `kernel.v1.capability.cancel` | partial | 内存 invocation registry 追踪进行中的 stream；cancel 标记 invocation 为 cancelled 并阻断后续 chunk。 |
| `kernel.v1.extension_point.list` | implemented | 列出已注册的 extension point。 |
| `kernel.v1.extension_point.describe` | planned | 注册表可以检查描述符；协议方法尚未暴露。 |
| `kernel.v1.hook.list` | partial | 协议分发器可以列出已注册的 hook；公开文档和更丰富的过滤仍为 Platform Host Alpha 工作。 |
| `kernel.v1.asset.put/get/list` | partial | 对 host-dev 协议调用者存在不透明 asset 底座，可以从 SQLite 支持的事件 rehydrate。Package-principal 权限检查和内容寻址 blob 存储仍为下一步。 |
| `kernel.v1.projection.register/rebuild/get` | partial | 通用 projection 注册表存在，可以从 SQLite 支持的事件 rehydrate；rebuild 当前从过滤后的事件流计算事件计数/最后序列号。包拥有的 projection 执行仍为下一步。 |
| `kernel.v1.host.info` | implemented | 返回协议版本、已公布的方法及其状态，以及当前支持的跨 in-process、HTTP `/rpc`、host stdio 和 ad hoc HTTP 的传输层标签。 |
| `kernel.v1.host.ping` | partial | 已公布；直接 service 路由尚未暴露。 |
| `kernel.v1.host.diagnostics` | partial | 返回包/capability/hook 计数和包记录，用于本地 host 可观测性。 |
| `kernel.v1.host.principal` | planned | Identity provider 集成 deferred。 |
| `kernel.v1.permission.grant/revoke/list/audit` | partial | Host-dev 调用者可以向 human 或 assistant principal 授予/撤销有范围权限、列出授权、检查授予/撤销审计事件。授权/撤销事件可从持久事件日志 rehydrate。完整资源策略覆盖仍为下一步。 |
| `kernel.v1.proposal.create/get/list/approve/reject/apply` | partial | 用于需要审批的游创变更的通用 proposal lifecycle。初始 apply 支持覆盖 `asset.put` 和 `projection.rebuild`；proposal operations/expected effects 中的 raw secrets 会被拒绝；更广泛的事务和 revert/compensation 仍为下一步。 |
| `kernel.v1.surface.contribution.list` | partial | 列出类型化的包声明 surface 描述符，涵盖 experience entry、Home/Play、Forge、asset 编辑器和 assistant slot。内核仅存储描述符；UI 渲染和内容语义仍为包/客户端工作。 |
| `kernel.v1.surface.contribution.describe` | partial | 按 id 描述一个已声明的 surface contribution。 |

## 内核事件类型矩阵

| 事件类型 | Writer | 状态 | 触发 |
|---|---|---:|---|
| `kernel/v1/session.opened` | kernel | implemented | Session 开启。 |
| `kernel/v1/session.closed` | kernel | implemented | Session 关闭。 |
| `kernel/v1/session.forked` | kernel | implemented | Session fork 创建 branch 族系。 |
| `kernel/v1/package.loaded` | kernel | implemented | Manifest 被接受并注册。 |
| `kernel/v1/package.loading` | kernel | implemented | 包记录进入 loading。 |
| `kernel/v1/package.starting` | kernel | implemented | Subprocess 包进程即将启动/握手。 |
| `kernel/v1/package.ready` | kernel | implemented | 包在 entry 特定启动后就绪。 |
| `kernel/v1/package.stopping` | kernel | implemented | Unload/restart 正在停止包执行。 |
| `kernel/v1/package.stopped` | kernel | implemented | 包执行已停止。 |
| `kernel/v1/package.unloaded` | kernel | implemented | 包从注册表移除。 |
| `kernel/v1/package.degraded` | kernel | implemented | 实际包执行失败/健康丢失。 |
| `kernel/v1/package.log` | kernel | implemented | 捕获的 subprocess stderr 日志行。 |
| `kernel/v1/asset.put` | kernel | implemented | 不透明 asset 已存储。 |
| `kernel/v1/projection.updated` | kernel | implemented | 通用 projection 状态已 rebuild。 |
| `kernel/v1/capability.invoked` | kernel | planned | 调用 lifecycle 事件。 |
| `kernel/v1/capability.completed` | kernel | planned | 调用成功事件。 |
| `kernel/v1/capability.failed` | kernel | planned | 调用失败事件。 |
| `kernel/v1/stream.started` | kernel | partial | Streaming invocation 已启动。 |
| `kernel/v1/stream.chunk` | kernel | partial | Streaming chunk frame 已发出。 |
| `kernel/v1/stream.progress` | kernel | partial | Streaming 进度指示。 |
| `kernel/v1/stream.ended` | kernel | partial | Streaming invocation 正常结束。 |
| `kernel/v1/stream.error` | kernel | partial | Streaming invocation 发生错误。 |
| `kernel/v1/stream.cancelled` | kernel | partial | Streaming invocation 被调用者取消。 |
| `kernel/v1/stream.timeout` | kernel | partial | Streaming invocation 超时。 |
| `kernel/v1/permission.denied` | kernel | implemented | 权限拒绝审计。 |
| `kernel/v1/permission.granted` | kernel | implemented | 权限授予审计。 |
| `kernel/v1/permission.revoked` | kernel | implemented | 权限撤销审计。 |
| `kernel/v1/proposal.*` | kernel | partial | Proposal lifecycle 审计事件。 |
| `kernel/v1/outbound.request` | kernel | partial | 出站网络请求被允许并审计。 |
| `kernel/v1/outbound.denied` | kernel | partial | 出站网络请求被拒绝。 |
| `kernel/v1/error` | kernel | planned | 通用结构化内核错误事件。 |

非内核事件类型必须以 writer package id 加 `/` 开头。内核必须拒绝包尝试写入 `kernel/v1/...` 或其他包的 namespace。
...
| `network.hosts` | partial | 包在 manifest 中声明允许的出站 host；runtime `check_network_policy` 和 `check_and_audit_outbound` 为 Ygg 提供的 network helper 强制执行 allowlist。支持扁平 `hosts` 列表和结构化 `declarations`（host、methods、purpose）。官方包无绕过。被拒绝的请求写入 `kernel/v1/outbound.denied`；被允许的请求写入 `kernel/v1/outbound.request` 并带 redacted audit。 |
...
5. 权限拒绝写入 `kernel/v1/permission.denied` 审计事件。
...
25. 被拒绝的出站请求写入 `kernel/v1/outbound.denied` 事件；被允许的请求写入 `kernel/v1/outbound.request` 事件。两者可通过 `kernel.v1.outbound.audit` 检查。
...
27. `kernel.v1.capability.stream` 在启动流式调用前验证 descriptor 中 `streaming=true`。非流式能力（descriptor `streaming=false`）会被拒绝。
...
29. Streaming 生命周期按序发出 kernel 事件：`kernel/v1/stream.started`（启动）、`kernel/v1/stream.chunk`（chunk）、`kernel/v1/stream.progress`（进度）、`kernel/v1/stream.ended`（正常结束）、`kernel/v1/stream.error`（错误）、`kernel/v1/stream.cancelled`（取消）、`kernel/v1/stream.timeout`（超时）。
