...
- `kernel.capability.invoke` / `kernel.capability.stream` 启动或推进 agent 类包能力。
- `kernel.capability.cancel` 取消运行中的流式调用。
- `kernel.capability.discover` / `kernel.capability.describe` 发现可映射为 agent 工具的能力。
- `kernel.proposal.create/get/list/approve/reject/apply` 承载 agent 产生的变更提案。
- `kernel.event.append/list/subscribe` 承载包拥有的 trace、工具调用和 run events。
- `kernel.surface.contribution.*` 让 Assist / Forge 通过公开协议发现 agent 动作和 trace 面板。
...
| cancellation | `kernel.capability.cancel` | 使用通用流式取消生命周期。 |
| tool discovery | `kernel.capability.discover` / `describe` | 工具是能力的 adapter view。 |
| tool execution | `kernel.capability.invoke` / `stream` | 必须保留调用者身份、provider 包、权限门禁和审计。 |
| tool ambiguity | 显式 `provider_package_id` | 禁止自动偏向官方 provider。 |
| proposal | `kernel.proposal.*` | Agent 不直接修改受信状态。 |
| trace | 包拥有的事件或流式帧 | 内核不理解 trace payload。 |
| state | 包拥有的 asset/projection/get_state 能力 | 不新增 `kernel.agent.state`。 |
