...
平台基础已经就位。Yggdrasil 现在拥有内容无关的内核、基于 manifest 的包、真正的 `rust_inproc` 和 subprocess 执行、权限/principal 系统、hook fabric 切片、surface 贡献、proposal/approval lifecycle、asset/branch/projection 底座、官方基础包、assistant 包、空白游创循环以及走公开协议的 Home/Play 和 Forge surface 的 Web shell。

下一个重心**不是**更多底座。而是收敛，然后是第一个真正可游玩的体验，端到端地演练底座。

## 当前位置

- Platform Foundation Alpha：已完成。
- Play/Forge Surface Contract Beta：已完成（51 个 conformance 用例）。
...
## Phase F — Foundation Alpha 收敛（当前）
...
## Phase G — Playable Experience Alpha（下一步）

目标：通过构建一个真正可游玩、可修改、可 fork、可由 assistant 辅助的体验来证明底座，全部作为普通包实现。

这是平台第一次产出游创创作者可以坐下来体验超过一个 demo 的东西。它不是 SillyTavern，不是纯对话运行时，不是 director —— 它是最小的、诚实演练每个底座原语的体验。

体验本身是包的决定，不是内核的决定。一个合理的候选是一个小型结构化角色扮演或独坐叙事循环，但选择是开放的，将在本阶段开始时决定。
...
## Phase H — Authoring & Composition Beta+（G 之后）
...
- 一份小型上手指南，带领贡献者从 `cargo run -p ygg-cli -- init-package ...` 到第三方包出现在 Home 和 Forge 中。
...
Phase F 仅为收敛 —— 不增加新底座。Phase G 是底座之上第一个真正的产品验证。Phase H 在底座至少有一个非平凡消费者后跟进。Phase I 在后台运行，以 charter 纪律评分（无内容形态泄漏到内核，无官方特权通过任何路径泄漏）。
