...
- Experience-Led Platform Beta: long-term direction; design in [`docs/product/EXPERIENCE_LED_PLATFORM_BETA.md`](../product/EXPERIENCE_LED_PLATFORM_BETA.en.md). Experience Beta 0–6, Performance & Code Health Beta, and External Project Operating Plane Alpha are complete. The current line is Storage Backend Neutrality Alpha; temporary plan: [`docs/roadmap/STORAGE_BACKEND_NEUTRALITY_ALPHA.en.md`](./STORAGE_BACKEND_NEUTRALITY_ALPHA.en.md). Goal: make storage contracts backend-neutral before expanding toward PostgreSQL, TDB, or retrieval backends.
