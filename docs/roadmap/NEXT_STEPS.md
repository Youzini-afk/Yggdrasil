...
- Experience-Led Platform Beta：长期方向；长期设计见 [`docs/product/EXPERIENCE_LED_PLATFORM_BETA.md`](../product/EXPERIENCE_LED_PLATFORM_BETA.md)。Experience Beta 0–6、Performance & Code Health Beta、External Project Operating Plane Alpha、Storage Backend Neutrality Alpha、PostgreSQL + TDB Integration Alpha 已完成。当前主线是 Real TDB Rust Adapter Alpha，临时计划见 [`docs/roadmap/REAL_TDB_RUST_ADAPTER_ALPHA.md`](./REAL_TDB_RUST_ADAPTER_ALPHA.md)。Storage 指南见 [`docs/guides/STORAGE_BACKEND_NEUTRALITY.md`](../guides/STORAGE_BACKEND_NEUTRALITY.md)；PostgreSQL + TDB 指南见 [`docs/guides/POSTGRES_TDB_INTEGRATION.md`](../guides/POSTGRES_TDB_INTEGRATION.md)。
...
## External Project Operating Plane Alpha（已完成）
...
- **E3 — Managed Workspace Deterministic Proof**（已完成）：`official/workspace-lab` 扩展 7 项 deterministic fixture managed workspace 能力（create_fixture_workspace / inspect_workspace / read_workspace_metadata / plan_workspace_run / record_fixture_process_result / discover_workspace_entrypoints / draft_workspace_patch）。Deterministic fixture workspace descriptor 含 managed_workspace_kind="fixture"、execution_performed=false、workspace_created_in_host=false、真实创建需要 approval/policy/executor。无文件系统、无进程、无网络、无 shell。Patch 仅生成 proposal，含 unsafe path 阻断和 raw secret 阻断。Entrypoint discovery 来自 stack_hint/metadata/scripts。7 个新增 conformance 用例（完成后总计 267；本阶段最终总计 275）。
- **E4 — Web Project Aggregation UI**（已完成）：Home/Forge 显示 external projects、workspaces、risk、entrypoints、logs、adapter candidates，public protocol-only。
- **E5 — Adapter / Wrapper Generation Proof**（已完成）：`official/project-intake-lab` 扩展 4 项 adapter/wrapper 生成预览能力——`generate_adapter_manifest_preview`、`generate_subprocess_wrapper_preview`、`generate_adapter_fixture_preview`、`check_adapter_readiness`——证明未适配外部项目可通过普通 third-party Ygg package 路径包装。Adapter package id 不可为 `official/`；权限默认最小；wrapper 预览含要求 policy-gated executor 的安全注释；fixture 已 redacted；readiness checklist 覆盖 namespace、permissions、fixture、secrets 和 approval 需求。示例：`examples/packages/external-project-adapter-preview/`。8 个新增 conformance 用例（总计 275）。
- **E6 — Durable cleanup**（已完成）：删除临时计划，新增 [`docs/guides/EXTERNAL_PROJECT_OPERATING_PLANE.md`](../guides/EXTERNAL_PROJECT_OPERATING_PLANE.md)，并收敛 ALPHA_STATUS、NEXT_STEPS、CONFORMANCE_MATRIX、README。
...
Phase F、Phase G 的 seed 形态、Creative Capability Kit Alpha、Model Connectivity Kit Alpha、Code Health Split Alpha、Runtime Split Alpha、Authoring & Composition Beta+、Secure Execution Substrate Alpha、Optional Text Engine Alpha、Agent Infrastructure Alpha、Model Provider Integration Alpha、Live Model Calls Alpha、Creative Inference Capability Alpha、Agentic Forge Beta、Experience Beta 0、Experience Beta 1、Experience Beta 2、Experience Beta 3、Experience Beta 4、Experience Beta 5、Experience Beta 6、Performance & Code Health Beta 和 External Project Operating Plane Alpha 已完成。所有后续阶段都以 charter 纪律评分：无内容形态泄漏到内核，无官方特权通过任何路径泄漏，所有 package/UI 行为都使用公开协议边界，并且新增 substrate 必须服务真实 playable experience 的压力。
