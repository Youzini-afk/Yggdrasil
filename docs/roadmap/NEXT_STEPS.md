...
- Experience-Led Platform Beta：长期方向；长期设计见 [`docs/product/EXPERIENCE_LED_PLATFORM_BETA.md`](../product/EXPERIENCE_LED_PLATFORM_BETA.md)。Experience Beta 0–6、Performance & Code Health Beta、External Project Operating Plane Alpha 已完成。当前主线是 Storage Backend Neutrality Alpha，临时计划见 [`docs/roadmap/STORAGE_BACKEND_NEUTRALITY_ALPHA.md`](./STORAGE_BACKEND_NEUTRALITY_ALPHA.md)。目标是在真正扩展 PostgreSQL/TDB/检索后端前，先确保 storage contract backend-neutral。
