...
- **Human-testing substrate:** install warnings and schema shapes are stable; native project install now flows source → store → nested manifests/profile autoload → project registry → project dist → protected `/surface-bundles/projects/<project_id>/...` → a short-lived sandbox asset lease; `surface_bundle` is a static, non-executing entry; `dist/` is included in `tree_hash`, store schema migration clears old stores, and install/update/uninstall garbage-collect orphan stores; `official/install-lab` provides `check_for_updates` / `update_project`, and both CLI `yg update` and the web project console route through it; the Surface bridge has converged on allowlists, stream ownership, redacted diagnostics, secret-input cleanup, CSP/CORS hardening, and typed `allowed_capability_ids`; Desktop manages a loopback Host sidecar and the Web shell is installable as a PWA; self-hosted deployment includes target / exec / port / proxy primitives, HTTP/WebSocket reverse proxy, the explicit Deploy broker, private-by-default / explicitly public routes, and revocable scoped-device pairing so phones control projects, deployments, and ChangeSets through the same Host API.
...
- The Experimental World Bundle is implemented and proven with `official/playable-creation-board`: canonical archive descriptors cover exact v1 envelopes and the complete SHA-256 closure; fresh SQLite/filesystem hosts preserve objects, lineage, and receipts; historical replay invokes no executor; replacement execution creates a child branch/head; and the headless CLI reads the same archive without Web-shell state.
- A JSON Schema subset validates capability I/O and package-declared event payloads.
- Principals: `host_admin`, `host_dev`, `host_device`, `package`, `human`, `assistant`, `anonymous`. Device principals retain grant, delegation, and resource authority across the remote RPC boundary instead of degrading to `host_dev`; human and assistant principals get scoped grants.
- Audit events: `kernel/v1/permission.granted|revoked|denied`, `kernel/v1/package.*` lifecycle, and `kernel/v1/proposal.*` lifecycle; the Host control plane also writes redacted `host/control/v1/authority.decision` records outside Contract V1.
- Persistent grants: grant / revoke events rehydrate inside a SQLite-backed runtime.
- Contract V1 is the public platform spec: 80 protocol methods, 59 event kinds, and 161 JSON Schemas. `kernel.v1.cap.*`, `kernel.v1.audit.package`, capability handles, binding injection, Path B, the conformance kit, and SDK generation are implemented.
...
- Deployment runtime: `kernel.v1.target.*`, `kernel.v1.exec.*`, `kernel.v1.port.*`, and `kernel.v1.proxy.*` are implemented; default is deny-all; profiles can opt into `LiveLocalExecExecutor`; ports are loopback-only; proxy upstreams must reference active port leases; `ProxyRouteAccess` defaults to `host_authenticated`, and only an explicit `public` route enables the optional unauthenticated `<slug>.apps.<host>/` vhost, while `/p/<route_id>/...` remains inside Host authentication. The web project console can explicitly Deploy / Stop Docker from `project.metadata.deployment.docker`, or run Dockerfile / nixpacks source Build & Deploy from `project.metadata.deployment.build_deploy` with runtime env, approved volumes, job SSE, and cancellation. Both paths make route exposure explicit and persist it in durable revisions.
...
| exact-project-gated `/surface-bundles/projects/<id>/<file>` route | implemented |
