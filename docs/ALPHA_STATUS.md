...
- **Conformance：** 124 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Agent Infrastructure Alpha 与 Model Provider Integration Alpha 已完成并文档化；Live Model Calls Alpha L3 已实现（`kernel.outbound.execute` 公开协议方法：package principal 通过 context 确定 package_id 不能 spoof、FakeOutboundExecutor + allowed network declaration 成功、response executor_kind Fake/network_performed false、host audit 产生；spoofed package_id 被拒绝；无 network permission denied 且 executor 不调用；secret_refs 仅引用 no raw secret in response），仍需 L4+ 推进真实 provider live call，且必须通过普通 provider package 路径，不能变成 kernel model/prompt 语义或中转站后台。
