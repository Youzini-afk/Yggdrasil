...
- Contract V1 is the public platform spec; all 160 schemas (80 methods + 59 events + 21 top-level) validate, and 473 conformance cases pass.
- The first eight layering-migration steps are complete: v1 factual-drift repair, the Experimental Contract Registry with explicit negotiation, 36 owner-namespace canonical/legacy dual stacks, the SHA-256 object/artifact foundation, EffectReceipt / Change primitives, the Protocol Commons scaffold, independent package-envelope/component identity with trust and composition pins, and the real World Bundle portability proof. The remaining migration step in [`CONTRACT_V2_MIGRATION.md`](CONTRACT_V2_MIGRATION.en.md) moves clients to canonical APIs and completes one real deprecation transition.
