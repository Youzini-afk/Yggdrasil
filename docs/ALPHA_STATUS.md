...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1 + Experience Beta 2 + Experience Beta 3 + Experience Beta 4 + Experience Beta 5 + Experience Beta 6 + Performance & Code Health Beta（已完成）+ External Project Operating Plane Alpha Phase E5（已完成；E6 执行中）。
...
- **当前主线：** External Project Operating Plane Alpha Phase E5 已完成。`official/project-intake-lab` 现提供 adapter/wrapper 生成预览能力——`generate_adapter_manifest_preview`、`generate_subprocess_wrapper_preview`、`generate_adapter_fixture_preview` 和 `check_adapter_readiness`——证明未适配外部项目可通过普通 package 路径包装成 Ygg package，无需原项目先适配。Adapter 必须是普通 third-party package（不可为 `official/`），权限默认最小，wrapper 包含要求 policy-gated executor 的安全注释，fixture 已 redacted，readiness 提供完整检查清单。Phase E6（持久文档收敛）下一步。临时计划见 `docs/roadmap/EXTERNAL_PROJECT_OPERATING_PLANE_ALPHA.md`；外部调研证据位于 `/tmp/opencode/ygg-external-project-plane-20260520/`。红线：external project 不是 package，managed workspace 不是 kernel object，adapter/wrapper 才是普通 package；危险动作必须 policy/proposal/audit gated。
