...
- **阶段：** Platform Foundation Alpha + Play/Forge Surface Contract Beta + Secure Execution Substrate Alpha + Optional Text Engine Alpha + Agent Infrastructure Alpha + Model Provider Integration Alpha + Live Model Calls Alpha + Creative Inference Capability Alpha + Agentic Forge Beta + Experience Beta 0 + Experience Beta 1。
- **Conformance：** 197 个具名 CLI 用例，加上 crate 和 service 单元测试。
...
- **当前主线：** Experience Beta 1 已完成；Yggdrasil 已具备首个真实 playable vertical slice——`official/playable-creation-board`，包含 board/module/constraint/marker state、player action recording、structured change requests、agentic forge binding、checkpoint/recovery、provenance chain——全部作为普通包 artifact，无内核体验语义。下一阶段方向为 **Experience Beta 2**：让体验状态和生成资产真正可追踪、可比较、可恢复。
...
- `official/playable-creation-board` —— Experience Beta 1：首个真实 playable vertical slice。Package-owned playable creation board，包含 board/module/constraint/marker state。提供 describe_contract（列出所有 11 项能力、lifecycle 状态、player action kinds、change kinds、checkpoint 格式、recovery 策略、risk levels 与 surface/capability shape）、launch（启动带初始 board state 的 creation board）、project_state（投影当前 board state 为 package-owned opaque state snapshot）、render_payload（产生 protocol-visible board render payload）、record_player_action（记录 player action，产生 state_delta_asset_ref/projection_ref/sequence/provenance）、request_change（产生 structured agent objective / allowed_change_kinds / risk/budget / bindable refs，不是聊天消息）、create_checkpoint（产生含 state_snapshot/asset_refs/branch_ref/sequence 的确定性 board checkpoint）、inspect_checkpoint（验证 board checkpoint shape 并报告错误）、draft_recovery（产生含推荐策略、步骤与审批要求的 recovery plan）、bind_agent_run（产生将 board session 链接到 agentic-forge 的 scoped agent-run binding）、explain_provenance（产生 player_action_event→state_delta_asset→checkpoint→agent_run→candidate→proposal→projection_rebuild 因果链）。4 个 surface：experience_entry、play_renderer、forge_panel、assistant_action。Raw-secret blocking。无 inference、无 network、无 kernel.experience/world/scene/character/turn/chat/memory/agent/model/prompt/director 命名空间。10 个 conformance 用例。第三方 agentic-forge 替换 composition 证明无 official priority。
