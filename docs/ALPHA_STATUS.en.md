...
- **Conformance:** 473 named CLI cases pass, plus crate and service unit tests; 160 v1 schemas validate (80 methods + 59 events + 21 top-level).
...
- Contract V1 is the public platform spec: 80 protocol methods, 59 event kinds, and 160 JSON Schemas. `kernel.v1.cap.*`, `kernel.v1.audit.package`, capability handles, binding injection, Path B, the conformance kit, and SDK generation are implemented.
...
- Experimental Contract Registry `0.2.0` is implemented: the Host Control Plane, host bundle resolver, Shell contributions, Change/Proposal, and Projection now publish 36 canonical/legacy identity aliases, all reaching the original handlers. `ygg.shell.default/v1` can be negotiated explicitly, while Web may continue sending only old IDs. Generated SDKs expose canonical clients and legacy wrappers and reject ID/function/operation-ID collisions.
...
- Deployment runtime: `kernel.v1.target.*`, `kernel.v1.exec.*`, `kernel.v1.port.*`, and `kernel.v1.proxy.*` are implemented; default is deny-all; profiles can opt into `LiveLocalExecExecutor`; ports are loopback-only; proxy upstreams must reference active port leases; ygg-service provides HTTP/WebSocket reverse proxy with `/p/<route_id>/...` and optional `<slug>.apps.<host>/` virtual-host entry. The web project console can explicitly Deploy / Stop Docker from `project.metadata.deployment.docker`, or run Dockerfile / nixpacks source Build & Deploy from `project.metadata.deployment.build_deploy` with runtime env, approved volumes, job SSE, and cancellation.
...
- `docs/spec/v1/schemas/` is the single source of truth for SDKs and conformance: 80 methods, 59 events, 21 top-level schemas, 160 total.
...
`cargo run -p ygg-cli -- conformance` runs 473 named CLI cases. Flags:
