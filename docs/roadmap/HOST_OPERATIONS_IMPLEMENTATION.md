...
实施结果（2026-07-24）：全部切片已经闭环。target 端口继续只绑定 loopback；HTTP 与 WebSocket 流量只通过认证 reverse tunnel，private route 保持 Host 身份门禁，public vhost 必须来自显式 `public` route。GitHub CI 覆盖重复连接所有权、HTTP framing、vhost authority、重连、revoke、backpressure、stale epoch 与持久 readiness 再投影。
